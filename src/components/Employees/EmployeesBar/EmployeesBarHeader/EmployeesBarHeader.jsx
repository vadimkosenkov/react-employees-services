...
class EmployeesBarHeader extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isRowView: false,
  ...
  changeView = () => {
    this.props.changeView(!this.isRowView);
  ...
  render() {
    return (
      <div className={s.employeesBarHeader}>
        <span className={s.employeesDisplayed}>
          {this.props.employeeCount} employees displayed
        </span>
        <div className={s.viewBar}>
          <svg
            className={
              s.gridViewIcon + " " + (this.state.isRowView ? "" : s.svgActive)
            }
          ...
          <svg
            className={
              s.rowViewIcon + " " + (this.state.isRowView ? s.svgActive : "")
            }
            onClick={this.changeView}
            width="24"
            height="24"
